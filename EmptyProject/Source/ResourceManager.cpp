...
//!	@return		: example
//-------------------------------------------------------------
IResource* ResourceManager::GetResouce(const std::string& name, RESOURCE_TYPE type)
{
	...
	{
	case PRIMITIVE_BOX:
		m_ResouceList[name] = m_BoxFactory->CreateBox();
		break;
	case PRIMITIVE_SPHERE:
		m_ResouceList[name] = m_SphereFactory->CreateSphere();
		break;
	default:
		return NULL;
	}
	return m_ResouceList[name]->Clone();
}
//-------------------------------------------------------------
//!	@brief		: example
//!	@param[in]	: example
