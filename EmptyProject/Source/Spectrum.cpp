...
void Spectrum::CreateSpectrumData()
{
	...
	for (U32 i = 0; i < vertexNum; i += 4)
	{
		...
		Spectrum[0].x	= (sideOffset + (onePanelSizeX * ((i / 4) + 1) - (margin * 0.5f))) * windowWidth;
		Spectrum[0].y	= 0.98f * windowHeight;
		Spectrum[0].z	= 0.f;
		Spectrum[0].w	= 0.f;
		Spectrum[0].u	= 1.f;
		Spectrum[0].v	= 0.f;
		Spectrum[0].color	= 0x00FFFFFF;

		Spectrum[1].x	= Spectrum[0].x;
		Spectrum[1].y	= (topOffset) * windowHeight;
		Spectrum[1].z	= 0.f;
		Spectrum[1].w	= 0.f;
		Spectrum[1].u	= 1.f;
		Spectrum[1].v	= 1.f;
		Spectrum[1].color	= 0x00FFFFFF;
		...
		Spectrum[2].u	= 0.f;
		Spectrum[2].v	= 0.f;
		Spectrum[2].color	= 0x00FFFFFF;

		Spectrum[3].x	= Spectrum[2].x;
		Spectrum[3].y	= Spectrum[1].y;
		Spectrum[3].z	= 0.f;
		Spectrum[3].w	= 0.f;
		Spectrum[3].u	= 0.f;
		Spectrum[3].v	= 1.f;
		Spectrum[3].color	= 0x00FFFFFF;
...
void Spectrum::Update(F32* data, U32 size)
{
	...
	{
		U32 a = i / 4;
		m_Vertex[i+1].y = m_Vertex[i+3].y = ((1.f - 0.1f) - (FData[i / 4] * 2.f)) * WINDOW_HEIGHT;
	}
	m_VertexBuffer->Lock(0, NULL, (void**)&data, 0);
	memcpy(data, &m_Vertex[0], sizeof(SpectrumVertex) * m_Vertex.size());
	m_VertexBuffer->Unlock();
